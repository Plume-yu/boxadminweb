...
class OpMsg:
    ...
    def parse(self, serializedString, convertMode = ConvertMode.SENDER):
        ...
        if convertMode == ConvertMode.RECEIVER:
            gusid = receiver_gusid
        else:
            gusid = sender_gusid
        ...
        self.arguments = self._decodeArguments(gusid, arguments)
        self.result_code = result_code
        self.result_scalar = self._decodeResultScalar(gusid, result_scalar)
        self.result_sets = self._decodeResultSets(gusid, result_sets)
        self.blob = blob
    ...
    def serialize(self, convertMode = ConvertMode.RECEIVER):
        if convertMode == ConvertMode.SENDER:
            gusid = self.sender_gusid
        else:
            gusid = self.receiver_gusid
        ...
        return OpMsgEncoder.serialize(self.sender_gusid,\
                                      ...
                                      self.session_key,\
                                      self._encodeArguments(gusid, self.arguments),\
                                      self.result_code,\
                                      self._encodeResultScalar(gusid, self.result_scalar),\
                                      self._encoderResultSets(gusid, self.result_sets),\
                                      self.blob)
...
class PingAnswer:
    ...
    def addOptValues(self, value):
        self.opt_values.append(value)
    
    def serialize(self):
        msg = PingAns()
        msg.time = self.time
        msg.value1 = self.value1
        msg.value2 = self.value2
        for value in self.opt_values:
            msg.opt_values.append(value)
        
        return msg.SerializeToString()

__all__ = ["JobType", "ExecType", "OpMsg", "hexdump", "PingAnswer"]
