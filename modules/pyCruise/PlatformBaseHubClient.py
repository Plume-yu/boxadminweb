...
from OpMsg import JobType, OpMsg, PingAnswer
from ServerCategory import GUID
import time
...
class PlatformBaseHubClientProtocol(PlatformBaseProtocol):
    ...
    def parseRecvMessageReq(self, msg):
        ...
        if msgID == 1:
            ...
            serverCategory = GUID.getCategory(serverID)
            
            if serverCategory == 13:
                if msgID == 12:
                    answer = PingAnswer()
                    answer.setTime(int(time.time()))
                    answer.setValue1(0)
                    answer.setValue2(0)
                    self._sendMessage(serverID, jobID, 13, answer.serialize())
                else:
                    self.factory.log.Info("It's not ping request message")
            else:
